...
SummarizationCallback = Callable[[Summarization], None]
...
def process_meeting_audio(
    ...
    logger.info(
        "pipeline_stage_started job_id=%s stage=%s", job_id, PipelineStage.PREPROCESSING
    )
    _report_progress(progress_callback, PipelineStage.PREPROCESSING, 15)
    normalized_audio_path = normalize_audio(audio_path)
    logger.info(
        "pipeline_stage_completed job_id=%s stage=%s",
        job_id,
        PipelineStage.PREPROCESSING,
    )

    logger.info(
        "pipeline_stage_started job_id=%s stage=%s", job_id, PipelineStage.TRANSCRIPTION
    )
    ...
    logger.info(
        "pipeline_stage_started job_id=%s stage=%s", job_id, PipelineStage.DIARIZATION
    )
    ...
    logger.info(
        "pipeline_stage_started job_id=%s stage=%s", job_id, PipelineStage.ALIGNMENT
    )
    ...
    logger.info(
        "pipeline_stage_started job_id=%s stage=%s", job_id, PipelineStage.SUMMARIZATION
    )
    _report_progress(progress_callback, PipelineStage.SUMMARIZATION, 90)
    summary = summarize_meeting(aligned_transcript)
    if summarization_callback is not None:
        summarization_callback(summary)
    logger.info(
        "pipeline_stage_completed job_id=%s stage=%s",
        job_id,
